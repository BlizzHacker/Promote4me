...
console.log('Applying Promote4.me 2.9.6 clean WordPress plugin package...');
...
class P4ME_Proof_Manager {
  ...
  public static function init() {
    ...
    foreach (['processing','completed','on-hold'] as $status) {
      add_action('woo' . 'commerce_order_status_' . $status, function($order_id) use ($status) {
        $s = self::settings();
  ...
  public static function assets($hook) { if (strpos($hook, 'p4me') === false && $hook !== 'index.php' && $hook !== 'post.php') return; wp_enqueue_style('p4me-admin', plugins_url('assets/admin.css', __FILE__), [], '2.9.6'); }
...
== Changelog ==
= 2.9.6 =
* Added cache-safe clean package with neutral naming.
= 2.9.5 =
...
const out = zip(files);
write('public/downloads/promote4me-proof-manager.zip', out);
write('public/downloads/promote4me-proof-manager-2.9.6.zip', out);
write('public/downloads/p4me-proof-manager-296.zip', out);
let pkg = JSON.parse(fs.readFileSync('package.json','utf8')); pkg.version = '2.9.6'; if (!pkg.scripts.prebuild.includes('apply-2.9.6-wordpress-clean-package.mjs')) pkg.scripts.prebuild += ' && node scripts/apply-2.9.6-wordpress-clean-package.mjs'; fs.writeFileSync('package.json', JSON.stringify(pkg,null,2)+'\n');
let server = fs.readFileSync('server/index.js','utf8'); server = server.replace(/version: '[^']+'/g, "version: '2.9.6'"); fs.writeFileSync('server/index.js', server);
let app = fs.readFileSync('src/ProductApp.jsx','utf8'); app = app.replaceAll('promote4me-proof-manager.zip','promote4me-proof-manager-2.9.6.zip').replaceAll('promote4me-woocommerce.zip','promote4me-proof-manager-2.9.6.zip').replaceAll('WordPress / WooCommerce Pro','WordPress Proof Manager').replaceAll('WooCommerce','WordPress'); fs.writeFileSync('src/ProductApp.jsx', app);
console.log('Promote4.me Proof Manager 2.9.6 clean package generated.');
